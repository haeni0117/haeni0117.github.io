...
20210625 | NQEEN 내용복습 `Ch 13. 재귀 용법 - 01. 알고리즘 해결에 중요한 재귀 호출 이해` `(자료구조)힙구조 데이터삭제` `(자료구조)트리(Tree)-1~8` `#1931회의실배정(sort)` `#1026보물(sort)` `#10815숫자카드(sort)`| `backtracking` `heap` `que` `tree`| #10815문제가 계속 시간초과가 나서 다른 코드를 참조했는데 이진트리방법이어서 이진트리 공부 후 다시 도전하기
